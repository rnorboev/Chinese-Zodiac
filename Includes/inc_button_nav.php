<!DOCTYPE html>
<html>
<head>
	<title>button navigation</title>
</head>
<body>

<div class="nav">
<ul>
	<li><a href="index.php?page=home_page"><img src="images/HomePage.jpg" alt="[Home Page]" title="Home Page" style="border:0" /></a></li>
	<li><a href="index.php?page=site_layout"><img src="images/SiteLayout.jpg" alt="[Site Layout]" title="Site Layout" style="border: 0" /></a></li>
	<li><a href="index.php?page=control_structures"><img src="images/ControlStructures.jpg" alt="[Control Structures]" title="Control Structures" style="border: 0" /></a></li>
	<li><a href="index.php?page=string_functions"><img src="images/StringFunctions.jpg" alt="[String Functions]" title="String Functions" style="border: 0" /></a></li>
	<li><a href="index.php?page=web_forms"><img src="images/WebForms.jpg" alt="[Web Forms]" title="Web Forms" style="border: 0" /></a></li>
	<li><a href="index.php?page=midterm_assessment"><img src="images/MidtermAssessment.jpg" alt="[Midterm Assessment]" title="Midterm Assessment" style="border: 0" /></a></li>
	<li><a href="index.php?page=state_information"><img src="images/StateInformation.jpg" alt="[State Information]" title="State Information" style="border: 0" /></a></li>
	<li><a href="index.php?page=user_templates"><img src="images/UserTemplates.jpg" alt="[User Templates]" title="User Templates" style="border: 0" /></a></li>
	<li><a href="index.php?page=final_project"><img src="images/FinalProject.jpg" alt="[Final Project]" title="Final Project" style="border: 0" /></a></li>
</ul>
</div>
</body>
</html>